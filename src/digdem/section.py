...
class SectionPoints():
    ...
    def __init__(self,*args,normalized=False,z=[]):
        ...
        try:
            if len(args)==2:
                ...
                points=args[1]
                if type(points)==geom.LineString:
                    points=geom.MultiPoint(points.coords[:])
                    
                if type(points)==geom.MultiPoint:
                    if not points.is_empty:
                        self.points=points
                        self.pos=[linear_geom.project(point,normalized=normalized)
                                                    for point in points.geoms]
                        self.z=z
                elif type(points)==list:
                    self.pos=points
                    self.points=geom.MultiPoint([linear_geom.interpolate(s,normalized=self.normalized) for s in points])
                    self.z=z
                else:
                    warnings.warn('Second argument must be either shapely MultiPoint, LinearString or float list. \
                ...
                try:
                    assert(len(args)==0),'0 or 2 arguments must be given for SectionPoints creation'
                    self.set_empty()
    ...
    @z.setter
    def z(self,z):
        assert(type(z)==list),'z must be a float list'
        assert(all(type(zz) in [float,int,np.float32,np.float64] for zz in z)),\
                'z contains non int or float elements' 
        if type(self.pos)==list:
#            print(self.pos,z)
            assert((len(z)==0)|(len(z)==len(self.pos))),'length of z does not match points input'
        self._z=z
    ...
    def add_points(self,points,z=[]):
        ...
        for point in points:
            if type(point) in [float,int,np.float32,np.float64]:
                pos2.append(point)
                points2.append(geom.Point(self.line.interpolate(point,normalized=self.normalized)))
            elif point.isinstance(SectionPoints):
                try:
                    assert(point.line==self.line),'Combining SectionPoints along different lines'
                    pos2=pos2+point.pos
                ...
                pos2=pos2+[self.line.project(pp,normalized=self.normalized)
                                                    for pp in point]
...
class Section() :
    ...
    def __init__(self,points,name,orientation=None,direction='',
                 normalized=False,topography=None,name_sep='-',
                 fz_old=None,control_points=None,
                 dinterp=10,interp_spline_options={},
                 line_points_as_control_points=True,
                 point_name_dist=None):
        
        if type(points)==geom.linestring.LineString:
            self.line=points
        elif type(points)==list:
        ...
        if (control_points is None)&line_points_as_control_points:
            self.control_points=SectionPoints(self.line,self.line,
    ...
    def orientation(self,orientation):
        ...
        if type(orientation)==str :
            if orientation=='S-N':
                ...
                warnings.warn("orientation not set, must be either 'S-N', 'N-S', 'W-E' or 'E-W'")
            if reverse:
                self.line = shapely.ops.substring(self.line,
                                                  self.line.length, 0)
    ...
    def set_extreme_points_names(self,sep=None):
        ...
        if sep != '':
            ...
            if len(names)==2:
                ...
                warnings.warn('Extreme points names could not be set, invalid string separator')
            
    def set_extent(self,other):
        if other is not None:
            intersections=self.line.intersection(other)
            self.extent=SectionPoints(self.line,intersections,
                                      normalized=self.normalized)
            ...
            if self.fz_old is not None and len(self.extent.pos)>0:
                ...
                self.extent.z=list(self.fz_old(coords[:,0],coords[:,1],grid=False))
            
    def filter_control_points(self):
        if self.extent is not None:
            if (self.control_points is not None)&(len(self.extent.pos)>1):
                    self.control_points.filter_by_pos(self.extent.pos[0],
                                                      self.extent.pos[-1])
    ...
    def filter_intersection_points(self):
        if self.extent is not None:
            for key in self.intersection_points:
                if (self.intersection_points[key] is not None)&(len(self.extent.pos)>1):
                        self.intersection_points[key].filter_by_pos(self.extent.pos[0],
                                                          self.extent.pos[-1])
                if len(self.extent.pos)==0:
    ...
    def set_control_points(self,pos,z):
        try:
            ...
            self.control_points=SectionPoints(self.line,pos,z=z,normalized=self.normalized)
            self.filter_control_points()
        except AssertionError:
            raise
    ...
    def set_intersection_points(self,name2,pos,z=[]):
        try:
            assert(len(pos)==len(z)),'pos and z must have same length'
            self.intersection_points[name2]=SectionPoints(self.line,pos,z=z,normalized=self.normalized)
    ...
    def set_interp_spline(self):
        ...
        if len(ss)>0:
            ...
            npts=len(ss)
            self.interp_spline=scipy.interpolate.splrep(ss,zz,s=0,k=min(3,npts-1),
                                                        **self.interp_spline_options)
...
#                print(self.name,len(self.extent.pos))
            ...
            pos=list(np.arange(pos1,
                          pos2+self.d_interp/2,
                          self.d_interp))
            if pos[-1] != pos2:
                pos.append(pos2)
    ...
    def plot(self,axe=None,figsize=None,npos=200,
        ...
        if old_topo_properties is None:
            old_topo_properties={'lw':2,
                                 'color':'k',
                                 'linestyle':'-'}
        if new_topo_properties is None:
            new_topo_properties={'lw':2,
                                 'color':'r',
                                 'linestyle':'-'}
            
        if extent_properties is None:
            extent_properties={'linestyle':'None',
                               'marker':'o',
                               'ms':10,
                               'color':'g',
                               'mfc':'g'}
        ...
        if control_points_properties is None:
            control_points_properties={'linestyle':'None',
                                       'marker':'o',
                                       'ms':10,
                                       'color':'b',
                                       'mfc':'b'}
        ...
        if self.extent is not None and len(self.extent.pos)>0:
            ind=np.argwhere((pos>=self.extent.pos[0])
                            &(pos<=self.extent.pos[1]))
            ind=np.squeeze(ind)
        elif len(self.control_points.pos)>1:
            ind=np.argwhere((pos>=self.control_points.pos[0])
        ...
        if len(ind)>0:      
            def evaluate_new_topo(*args):
                if fz_new is None:
                    ...
                    out=fz_new(coords[ind[0]:ind[-1]+1,0],
    ...
    def plot_on_map(self,axe=None,figsize=None,point_name_dist=None,
             npts=2,section_properties={},text_properties={},
             control_points_properties=None):
        ...
        if len(self.extreme_points_names)>0:
            ...
            for i in [0,1]:
                ...
                axe.text(xx[-i]+point_name_dist*vecx,
