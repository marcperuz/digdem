...
class SurfMod(dict) :
    ...
    @staticmethod
    def read_shapefile(shapefile,direction_field_name='dir',
        ...
        try:
            ...
            for feature in layer:
                ...
                sections.append(Section(points,name,
                                        direction=direction,
    ...
    def read_control_points_from_textfile(textfile,sep=' '):
        ...
        with open(textfile,'r') as file:
            line=file.readline()
            try:
                assert(line[0]=='#'),'File must begin with a section name, identified with #'
                while line:
                    if line[-1]=='\n':
                        line=line[:-1]
                    if line[0]=='#':
    ...
    def __init__(self,surf_old,mask,xaxis=None,yaxis=None,sections=[],
                 contour=None,normalized=False,sparse_interp=1,dinterp=10,
                 ndv=-9999.9,smooth=0.5,surf_max=None,surf_min=None):
...
#                        'surf_old or mask are not np.ndarray'
    ...
    def set_fz_old(self,finterp=None,outside_mask=False):
        if finterp is None:
            ...
            zinterp=self.surf_old[indx[0]:indx[-1]+1:self.sparse_interp,
                                  indy[0]:indy[-1]+1:self.sparse_interp]
            if outside_mask:
                xinterp2,yinterp2=np.meshgrid(xinterp,yinterp,indexing='ij')
                #Indexs inside the mask
                mask2=self.mask[indx[0]:indx[-1]+1:self.sparse_interp,
                                  indy[0]:indy[-1]+1:self.sparse_interp]
                ind_mask=mask2>0
                #Indexes of mask contour
                mask3=np.copy(mask2)
                mask3[ind_mask]=1
                mask3=convolve2d(mask3,np.ones((self.sparse_interp,self.sparse_interp)),mode='same')
                ind_cont=(mask3>0)&(mask2==0)
                #Interpoling function using contour
                fz_tmp=scipy.interpolate.Rbf(xinterp2[ind_cont],
                                                  yinterp2[ind_cont],
                                                  zinterp[ind_cont],
                                                  function='multiquadric',smooth=self.smooth)
                #Modify altitudes within the mask for fz_old definition
                zinterp[ind_mask]=fz_tmp(xinterp2[ind_mask],
                                         yinterp2[ind_mask])
            self.fz_old=scipy.interpolate.RectBivariateSpline(xinterp,yinterp,zinterp)
        else:
            self.fz_old=finterp
    ...
    def set_fz_new(self):
        if len(self.interp_points.z)>0:
            self.fz_new=scipy.interpolate.Rbf(self.interp_points.xy[:,0],
        ...
        if len(args)==0:
            for i in range(nsections):
                for j in range(i+1,nsections):
                    ...
                    posi=[self[names[i]].line.project(point,normalized=self.normalized)
                                            for point in intersect.geoms]
                    posj=[self[names[j]].line.project(point,normalized=self.normalized)
                                            for point in intersect.geoms]
                    self.intersections[names[i]][names[j]]['pos']=posi
                    self.intersections[names[j]][names[i]]['pos']=posj
                    self.intersections[names[i]][names[j]]['z']=[np.nan for point in intersect.geoms]
                    self.intersections[names[j]][names[i]]['z']=[np.nan for point in intersect.geoms]
...
#                    print(names[j],names[i],posj)
            ...
            for key1,key2,z in zip(names1,names2,zs):
                if (key1 in names) and (key2 in names):
                    ...
                    self[key1].set_intersection_points(key2,
                                                        self.intersections[key1][key2]['pos'],
                                                        self.intersections[key1][key2]['z'])
                    self[key2].set_intersection_points(key1,
    ...
    def set_control_points(self,*args,sep=' '):
        ...
        if len(args)==2:
            ...
            pos,z,z_intersections=SurfMod.read_control_points_from_textfile(args[0],sep=sep)
...
#                        print(key,key2,self.intersections[key][key2]['pos'],self.intersections[key][key2]['z'])
    ...
    def set_interp_points(self):
        ...
        if self.contour is not None:
            ...
            interp_z=interp_z+[self.fz_old(coord[0],coord[1],grid=False) for coord in coords]

        #Add points from the sections
        for section in self.sections:
            section.set_interp_spline()
            section.set_interp_points()
            interp_points=interp_points+[pp for pp in section.interp_points.points.geoms]
            interp_z=interp_z+section.interp_points.z

        #Add supplementary control points
        interp_points=interp_points+[pp for pp in
                                     self.sup_interp_points.points.geoms]
    ...
    def plot(self,figsize=None,axe=None,
             contour_properties={},section_properties={},
             text_properties={},control_points_properties={},
             plot_interp_points=False,interp_points_properties={},
        ...
        #Plot topography
        digdem.plot.plot_topo(self.surf_new,
                              self.xaxis,
                              self.yaxis,
                              axe=axe,
                              **kwargs)
        ...
        #Plot sections
        for section in self.sections:
            section.plot_on_map(axe=axe,section_properties=section_properties,
